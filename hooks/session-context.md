<!-- EDITING THIS CARD: it is the always-on token budget (target ≤ ~1215 words; raised from 1150 in v0.26.0 for the descriptive arm, then +5 in v0.27.0 for the language-profile dimension — offset by compressing the memory section). Every addition must compress or evict something — it bloated 970→1256 once; don't let it creep again. GENERAL discipline lives here; PROJECT-specific facts live in docs/LESSONS.md + memory, not inline. -->
