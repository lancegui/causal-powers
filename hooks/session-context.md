...
For non-trivial work the plan is a **phased, checkboxed `analysis-plan.md`** — data ingest & cleaning is **Phase 1, owned by `data-preparation`** (the heaviest phase; plan it, don't treat it as one line), then construction → primary spec → robustness → verification.

**Keep it live; compact at phase boundaries.** Update it as you go; when a phase finishes (dataset validated, spec estimated, recovery passed, result verified), write the decisions + insight + concrete POST-COMPACT next steps into it and **offer to compact** ("clean point to `/compact` — the doc carries state forward"). Treat the plan as **disk-as-RAM** so the work is **resumable** — it survives `/clear` and compaction, and a clean session picks up from the file alone. Test: could a clean session resume from the doc alone? (`executing-analysis-plans`)
...
On **Claude Code**, the plugin's hooks run this automatically: a SessionStart injection (this block), a UserPromptSubmit keyword router + a PostToolUse skill-chain (trigger backstops), and a SessionStart/PreCompact `analysis-plan.md` resumability hook. On **Codex and other agents**, this file is your `AGENTS.md` and the skills load **natively** — they trigger off their `description` (same as Claude Code) or by explicit `$<skill-name>`; there are no hooks, so the discipline above is your always-on layer, and you maintain/flush `analysis-plan.md` yourself before compacting. Claude Code tool names in the skills (`Task`, `Skill`, `TodoWrite`) map to Codex equivalents — see [`skills/using-causal-powers/references/codex-tools.md`](skills/using-causal-powers/references/codex-tools.md).
