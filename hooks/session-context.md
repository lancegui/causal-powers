...
Threshold for both: *more than a couple of steps, or it touches sample/spec/design* → plan; a trivial one-liner you just do. **Wherever you're dropped in** mid-stream, reconstruct and confirm the plan first — an approved study design does **not** waive the task roadmap. Once approved, execute — an approved topology **is** fan-out consent (map it, don't re-ask; else offer fan-out, user's call). Durable state lives in **`docs/analysis/index.yaml`** plus small phase/decision/artifact YAML (Phase 1 = data ingest & cleaning, `data-preparation`), disk-as-RAM so a clean session resumes from the index without rereading a giant plan.
