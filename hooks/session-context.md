...
*Re-fire the discipline, not necessarily the file.* "Re-trigger" means **re-apply** the skill's discipline to this request and say so — not always re-invoke the Skill tool and reload the whole body. If you loaded the skill earlier this session and its guidance is still in context, just apply it ("re-applying `result-verification`: reconciling to source…"). Re-invoke the Skill tool only when its body has scrolled out, was compacted away, or you need the details again. The thing that must never be skipped is the *discipline*; reloading the text is optional.
...
## Always a written plan — at two altitudes — and a real approval gate

Don't carry the plan, spec, or model in your head. Plan at **whatever altitude the task is, and get agreement before executing:**

- **Study altitude** — "estimate the effect of X", "build this metric". Before any non-trivial work (*anything with an estimand / spec / sample / model decision in it, or beyond a single-file, single-function edit*), write the framing brief (`question-framing`), the pre-analysis plan for confirmatory work (`pre-analysis-plan`), or the structural **model card** (`structural-estimation`) and get sign-off.
- **Task altitude** — "merge these two messy sources", "diagnose why this number is off", "build this figure", "refactor this pipeline step". A multi-step chunk gets a **short numbered roadmap** stated and agreed *first* — the steps you'll take, in order — not a dive. This is the rung most often skipped: the work has no estimand decision so it feels like "just doing it", but a bisection or a merge **is** a multi-step plan, and one the user can't see is one they can't redirect (the local knowledge — "check Milwaukee first", "that join fans out" — arrives only if you show the roadmap). Same pattern as study altitude, one rung down. *Agree once, then loop autonomously within the roadmap*; only re-stop if a step turns into a design/sample/spec change (`analysis-checkpoints`). A genuinely trivial one-or-two-step edit you just do — the threshold is the same "more than a couple of steps, or touches sample/spec/design" line.

**Wherever you're dropped in**: "just estimate / fix / run / merge this" mid-stream still means write (or reconstruct) the plan or roadmap, confirm, *then* proceed — and an approved study design does **not** waive the task-level build roadmap. The spine: frame → (PAP if confirmatory) → **written plan + user approval** → execute (fan independent specs out to subagents) → verify → review. Do not barrel through the gate on your own reading.
