...
def assert_join(left, right, on, how="left", expect="m:1", allow_unmatched=False):
    """Merge with declared cardinality + row-count bracket + unmatched report."""
...
def na_audit(df, cols=None, max_new_na=0, baseline=None):
    """Report (and bound) missingness; pass a baseline dict to catch new NAs."""
...
function assert_join(left, right; on, how=:left, expect="m:1", allow_unmatched=false)
    ...
    if !allow_unmatched
        ...
        @assert nrow(bad) == 0 "$(nrow(bad)) rows unmatched on $on; sample: $(first(unique(bad[!,on]), 10))"
    end
    out
end
