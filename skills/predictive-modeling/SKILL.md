...
description: Use whenever the GOAL is a prediction rather than an effect — predict, score, rank, flag, classify, forecast, or detect-anomalies on units to drive an action ("which pharmacy is likely diverting opioids from its shipments", "which claim to audit", "which account to review", "flag the outliers", "rank by risk", "who's likely to churn"). The third arm of the modeling fork — route here by GOAL, not algorithm: if the deliverable is a prediction it is this skill even when a random forest does the work; if the goal is a causal effect it stays in causal-identification EVEN when ML does the heavy lifting (double/debiased ML, causal forests, ML propensity scores). Forces the prediction workflow — write and sign off the Prediction Spec (decision + cost asymmetry, target/label + regime, prediction-time, features + leakage audit, deployment-matched split, metric-tied-to-decision + baseline, method) BEFORE fitting; PROVE the evaluation is honest (permutation/null leakage probe, deployment-mirroring holdout, calibration, beat-the-baseline) before trusting any metric; and never read feature importance / SHAP as causal. Covers clean-label, proxy/weak-label, unsupervised/anomaly (no label), and ranking/triage regimes, in R, Julia, or Python — a high validation score is not a working model until leakage is ruled out and the split mirrors deployment.
---
...
- **Proxy / weak label.** You don't observe the thing; you observe a stand-in — "was audited and found" (not "was fraudulent"), "complaint filed" (not "harm occurred"). You are predicting the **proxy, not the thing**, and the spec must *say so*, because the proxy carries its own selection (you only see audits of units someone already suspected). Every metric is a metric on the proxy.
...
This is the proof step — the analog of structural estimation's Monte-Carlo recovery. There, you prove the *algorithm recovers truth* before believing it on real data; here, you prove the *evaluation is honest* before believing any score. It is `data-contracts` discipline applied to the **eval harness**: assert these properties, then **freeze the harness as a regression baseline** so a later refactor can't silently reintroduce leakage. None of this is optional, and all of it comes *before* you report a number.

- **Permutation / null probe.** Shuffle the labels and re-run the whole pipeline. Performance **must collapse to chance.** If a model trained on randomized labels still scores well, the harness is leaking — information is reaching the model through the plumbing, not the signal. This is the single highest-value check in the discipline; run it first.
...
- **No permutation/null probe** — the model trusted before randomized-label performance was shown to collapse to chance.
...
digraph predictive_modeling_next {
    ...
    "Prediction Spec written + approved?" -> "Goal is really an effect / a why?" [label="check first"];
    "Goal is really an effect / a why?" -> "back to causal-identification — wrong arm" [label="yes"];
    "Goal is really an effect / a why?" -> "invoke executing-analysis-plans — fan CV folds / candidate models / subsample cuts to subagents" [label="no — it's a prediction"];
...
1. **Get the Prediction Spec written and approved** — decision + cost asymmetry, label + regime, unit + prediction time, features + leakage audit, deployment-matched split, metric + baseline, method. This gate is mandatory before any fitting. If the goal turns out to be an effect or a "why," stop and route back to `causal-identification` — wrong arm.
