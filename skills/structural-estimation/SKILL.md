...
**Do this wherever you enter the pipeline — and you usually enter in the middle.** Users rarely start you at "frame the question"; they start you at "estimate this demand model", "fix the Monte-Carlo recovery", "recover the marginal costs", or "run the merger counterfactual". Being dropped at a middle step does **not** mean a spec exists or that the modeling is settled — it almost always means none was written. So the first move, *whatever the user asked for*, is to **write the spec (or reconstruct it from what they already have), surface it, and confirm — before doing the step they named.** Don't silently start at the step the user happened to point at; back up to the spec, then come forward. Jumping straight into the requested work because "they've clearly already decided" is the single most common way structural work goes un-specced.
...
**And the discipline is recurring, not a one-time gate.** Each major component — the Monte-Carlo recovery harness, the gradient, the estimation routine, each counterfactual — and every mid-stream fix to one ("speed up the gradient", "the estimator won't converge, sort it out") gets its own three-line mini-spec first: what it must do, what's wrong, exactly what changes (the true-θ grid, the recovery tolerance, the starting values, the sample size, the inner-loop tolerance), and what "fixed" looks like (it recovers θ from a distant start; the gradient matches finite differences). *Then* edit. A bare "fix it" is not a licence to skip the plan — it's where an un-specced change silently alters what's being estimated or quietly redefines what "recovery" counts as. A written mini-spec costs three lines; a non-spec'd "fix" costs a wrong counterfactual. The threshold is concrete: **anything you'd estimate at more than ~10 minutes of work gets a written mini-spec first** — only a genuine sub-10-minute surgical edit (a rename, a typo, a one-line tweak) skips it (`analysis-craft`).
...
- The user started you at a middle step — "estimate this demand model", "fix the recovery", "run the counterfactual" — and you began that work without first backing up to write or reconstruct the model spec and confirm it.
